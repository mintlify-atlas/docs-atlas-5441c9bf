...
description: "Configure Kopia Desk's backup behavior: enable file versioning, deep SHA-256 verification, content deduplication, exclusion filters, and the light/dark theme."
---
...
<Accordion title="Versioning — default ON">
  ...
  ```
  <destDrive>\KopiaDesk_Backup\.kopia-data\versions\<ISO-timestamp>\<relative_path>.gz
  ```

  Each backup run that overwrites at least one file creates a new subdirectory named with the ISO timestamp of that run (colons and dots replaced to keep the name filesystem-safe). The compressed file preserves the full relative path of the original, so you can always identify which file a `.gz` archive corresponds to.
...
To retrieve the default list programmatically via IPC:

```js
const defaults = await window.kopiaAPI.invoke("config:default-excludes");
```

To apply custom patterns for a specific scan, pass them as the second argument to `fs:scan-directory`:

```js
await window.kopiaAPI.invoke("fs:scan-directory", folderPath, [
  "*.tmp",
  "~$*",
  "node_modules",
  "build",
